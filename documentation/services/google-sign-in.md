...
5. The backend's authentication middleware (`internal/service/nethttp/middleware/authenticate.go`, wrapping every request) verifies the credential's signature, audience, and issuer using the [`google.golang.org/api/idtoken`](https://pkg.go.dev/google.golang.org/api/idtoken) package, resolves the verified profile to a `User` (`internal/objects/user.go`), and persists it via [Firestore-backed user storage](user-storage.md) — created on first sign-in, refreshed on every subsequent one — before attaching it to the request context. Route handlers — e.g. `GET /api/v1/hello` (`internal/service/nethttp/resources/v1/hello.go`) — read it back with `middleware.UserFromContext` to personalize behavior, but authentication itself isn't tied to any specific route.
6. Requests with no `Authorization` header are treated as anonymous (`middleware.UserFromContext` reports no user), not rejected — routes opt into requiring authentication themselves (none currently do).
