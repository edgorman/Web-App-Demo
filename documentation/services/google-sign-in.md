...
5. The backend's authentication middleware (`src/service/fastapi/middleware/authenticate.py`, registered on every request via `AuthenticationMiddleware`) verifies the credential's signature, audience, and issuer using the [`google-auth`](https://google-auth.readthedocs.io/) library, resolves the verified profile to a `User` (`src/objects/user.py`), and persists it via [Firestore-backed user storage](user-storage.md) — created on first sign-in, refreshed on every subsequent one — before attaching it to `request.user`. Route handlers — e.g. `GET /api/v1/hello` (`src/service/fastapi/resources/v1/hello.py`) — read `request.user` to personalize behavior, but authentication itself isn't tied to any specific route.
