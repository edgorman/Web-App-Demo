...
# Grant project developers permission to invoke the backend for testing/debugging
resource "google_cloud_run_v2_service_iam_member" "backend_developers_access" {
  depends_on = [google_cloud_run_v2_service.backend]
  ...
  project  = google_cloud_run_v2_service.backend.project
  role     = "roles/run.invoker"
  member   = "domain:${var.developers_domain}"
}
