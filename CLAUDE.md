...
- `src/objects/` holds pydantic data models (e.g. `Message`) — one object per file. Models that requests act on subclass `Resource` (`src/objects/resource.py`), overriding its empty `Action` enum and its deny-by-default `is_authorized(user, action)` to declare who may do what (e.g. `User`).
- `src/service/fastapi/middleware/authorize.py` enforces those rules per request: each protected route registers an `AuthorizationRule` (method, path template, action, resource resolver), and the middleware returns `403` when the resolved resource does not authorize `request.user`. It must be registered before the authentication middleware so it runs after it — see `documentation/services/authorization.md`.
