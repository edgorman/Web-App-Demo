...
class CICDEventUsecase:
    def __init__(
        ...
        self._workflow_submit_handler = workflow_submit_handler
    ...
    def run(self, start_commit: str, end_commit: str, event_type: GitEventType) -> None:
        ...
        try:
            files_changed = self._git_handler.get_files_changed(start_commit, end_commit)
        ...
        for file_changed in files_changed:
            try:
                cicd_files = self._git_handler.get_cicd_files(file_changed, event_type)
        ...
        for cicd_file in cicd_files:
            try:
                self._workflow_submit_handler.submit(cicd_file)
            except Exception as e:
